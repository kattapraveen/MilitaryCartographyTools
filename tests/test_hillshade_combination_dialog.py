...
"""
Tests for terrain/hillshade_combination_dialog.py's
generate_from_dialog_values() - the accept-flow logic split out of
show_hillshade_combination_dialog() so it can be exercised without
driving an actual modal QDialog. Mirrors
tests/test_hypsometric_tint_dialog.py's shape.
...
from MilitaryCartographyTools.terrain.hillshade_combination_dialog import (
...
class TestGenerateFromDialogValues(QgisTestCase):
    ...
    def test_regenerate_preserves_manually_moved_layer_position(self):
        ...
        # Real usability complaint (already fixed for Tanaka Contours/
        # Hypsometric Tint): correcting the layer in place is only
        # actually helpful if it doesn't also reset the layer's
        # position back to the default every time - the user is
        # likely to have dragged it somewhere deliberate.
        first = generate_from_dialog_values(self.iface, self._values())
    ...
    def test_azimuth_preset_value_reaches_the_generated_layer(self):

        result = generate_from_dialog_values(
            self.iface,
            self._values(azimuths=TWO_DIRECTION_AZIMUTHS)
        )
