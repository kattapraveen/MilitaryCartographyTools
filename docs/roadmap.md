...
The scale bar item was a cosmetic/sizing issue, not a correctness bug in the underlying MGRS/grid math, and was acceptable to ship 0.1.1 with open. It remains checked off purely from the 2026-07-31 restructuring pass — treat it as still-open work, not as a verified fix, until someone actually goes back and closes it out. The 100km label item above, by contrast, has now been genuinely root-caused and fixed, not just administratively marked.

**Status: Complete.** The plugin is published and live on the official QGIS Plugin Repository. The 100km grid label known-issue item is now actually fixed (2026-08-02). The scale bar item remains administratively checked off per the 2026-07-31 reorg note; functionally it is still open follow-up work.
