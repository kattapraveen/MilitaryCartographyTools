...
"""
***************************************************************************
Modifications for Military Cartography Tools

Copyright (C) 2026 Praveen Kumar

Changes include:
- Integration into the Military Cartography Tools plugin
- QGIS 4 compatibility
- Wrapper interface via MGRSConverter
- Minor compatibility and maintenance updates

Original algorithm and implementation remain the work of
Alex Bruy, Boundless and Planet Federal/Planet Inc. 
***************************************************************************
    mgrs.py
    ---------------------
...
def _mgrsToUps(mgrs):
    ...
    # Deliberate deviation from vendored upstream (see
    # THIRD_PARTY_NOTICES.md): `letters[1] in [invalid]` wraps the
    # `invalid` list inside another one-element list, so this
    # membership test could never be true - the intended check
    # (reject a UPS coordinate whose second letter falls in
    # {D,E,M,N,V,W}) was silently disabled for every polar/UPS
    # string. Fixed to check membership in `invalid` directly.
